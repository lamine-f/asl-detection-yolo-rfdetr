...
\midrule
mAP@0.5             & 0.904 & 0.958\textsuperscript{*} \\
mAP@0.5:0.95        & 0.876 & 0.901\textsuperscript{*} \\
Précision           & 0.879 & \TODO{TT} \\
Rappel              & 0.755 & \TODO{TT} \\
F1-score            & 0.812 & \TODO{TT} \\
Latence (ms/img)    & 23.5  & 141.2 \\
FPS (T4 GPU)        & 42.6  & 7.1   \\
Taille (\si{MB})    & 5.21  & 121.8 \\
\bottomrule
\end{tabular}\\
\smallskip
\footnotesize{\textsuperscript{*}~mAP de RF-DETR mesuré sur le set de validation (l'API \texttt{evaluate()} de \texttt{rfdetr} n'expose pas le test à l'écriture). L'évaluation explicite sur le test set est réalisée dans le notebook 04 via \texttt{supervision}.}
\end{table}
...
\textbf{Observation.} RF-DETR-S obtient un meilleur mAP en validation (0.958 contre 0.951 pour YOLO11n) et conserve cet avantage de précision sur les classes les moins fréquentes — un effet de la richesse sémantique du backbone DINOv2. YOLO11n conserve cependant un avantage massif en \emph{vitesse} (\textbf{\ensuremath{\sim}6$\times$ plus rapide}, 42.6~FPS contre 7.1) et en \emph{taille mémoire} (\textbf{23$\times$ plus léger}, 5.2~MB contre 121.8). Le compromis classique \emph{précision~/~efficacité} entre une architecture transformeur lourde et un détecteur convolutif compact est ainsi confirmé sur ce dataset.
