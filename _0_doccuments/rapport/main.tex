...
\subsection{Étude de robustesse}\label{sec:robust}
...
\noindent
La connexion HTTPS est nécessaire au fonctionnement de la webcam dans les navigateurs modernes (\texttt{getUserMedia} bloque les contextes non sécurisés).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_demo_screenshot.png}
\caption{Capture de l'application en mode \emph{comparaison sur image}. Le signe « Z » est exécuté à deux mains sur fond domestique, condition hors distribution par rapport au dataset d'entraînement. YOLO11n ne produit aucune détection en \SI{93}{\milli\second}, tandis que RF-DETR-S détecte la lettre avec une confiance de 0.92 en \SI{1070}{\milli\second}. Cet exemple qualitatif corrobore les conclusions de l'étude de robustesse (Section~\ref{sec:robust}).}
\label{fig:demo}
\end{figure}

\noindent
Le code source, les modèles et le rapport sont également disponibles publiquement~:
